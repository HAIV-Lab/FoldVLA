...
parser.add_argument(
    ...
    type=int,
    default=None,
    help="Interval between training videos in env steps. Defaults to one checkpoint/save stage.",
)
...
parser.add_argument(
    ...
    type=str,
    default="1920,1080",
    help="Video resolution as width,height.",
)
parser.add_argument(
    "--video_camera_eye",
    type=str,
    default="0.35,-0.45,0.82",
    help="Video camera eye position relative to the recorded environment origin.",
)
parser.add_argument(
    "--video_camera_lookat",
    type=str,
    default="-0.095,-0.005,0.62",
    help="Video camera target position relative to the recorded environment origin.",
)
...
def _parse_tuple_arg(value: str, name: str, length: int, cast=float):
...
def _configure_video_viewer(env_cfg: DirectRLEnvCfg):
    env_cfg.viewer.origin_type = "env"
    env_cfg.viewer.env_index = 0
    env_cfg.viewer.eye = _parse_tuple_arg(args_cli.video_camera_eye, "--video_camera_eye", 3)
    env_cfg.viewer.lookat = _parse_tuple_arg(args_cli.video_camera_lookat, "--video_camera_lookat", 3)
    env_cfg.viewer.resolution = _parse_tuple_arg(args_cli.video_resolution, "--video_resolution", 2, int)
    print(
        "[INFO] Video camera: "
...
def main(env_cfg: DirectRLEnvCfg, agent_cfg: dict):
    ...
    if args_cli.video:
        save_frequency = agent_cfg["algorithm"].get("save_frequency", 1)
        horizon_length = agent_cfg["algorithm"].get("horizon_length", 1)
        video_interval = args_cli.video_interval
        if video_interval is None:
            video_interval = max(1, int(save_frequency * horizon_length))
        video_folder = args_cli.video_dir if args_cli.video_dir is not None else os.path.join(log_dir, "videos", "train")
        video_kwargs = {
        ...
        print(f"[INFO] Recording training videos to: {video_folder}")
        print(f"[INFO] Video interval: {video_interval} env steps, video length: {args_cli.video_length} env steps")
