...
class Group(nn.Module):
    ...
    def forward(self, xyz, color):
        ...
        # fps the centers out
        center = fps(xyz, self.num_group).to(xyz.dtype)  # B G 3
...
class Uni3D(nn.Module):
    def __init__(
        self,
        vision_model_name: str = "eva02_tiny_patch14_224",
        pc_feat_dim: int = 192,
        group_size: int = 64,
        num_group: int = 512,
        pc_encoder_dim: int = 512,
        patch_dropout: float = 0.0,
        trans2embed: bool = True,
        embed_dim: int = 1024,
        single_output_token: bool = False,
        ...
        self.use_trans2embed = trans2embed
        self.single_output_token = single_output_token
        # grouper
        ...
        self.visual = timm.create_model(vision_model_name)
    ...
    def output_shape(self):
        ...
        group_num = self.num_group if not self.single_output_token else 1
        output_dim = self.trans_dim
        return (group_num, output_dim)
    ...
    def forward(self, pc):
        ...
        for i, blk in enumerate(self.visual.blocks):
            x = blk(x)
        if self.single_output_token:
            x = x[:, 0, :].unsqueeze(1)  # B 1 C
        if self.use_trans2embed:
