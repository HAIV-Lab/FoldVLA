...
`chumpy` is an old package whose build script imports `pip`, so `--no-build-isolation` is required with recent pip versions. Without it, installation may fail while building `chumpy` with `ModuleNotFoundError: No module named 'pip'`.
